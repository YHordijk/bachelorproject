...
############################################ Colour map for data colouring in excel
...
#for example:
# 	categories = ['a', '']
# 	files are 'a1.t21', 'a2.t21', 'b1.t21', 'b2.t21'
#will give:
#	kff_by_cat = {'a': ('a1.t21', 'a2.t21'), '': ('a1.t21', 'a2.t21', 'b1.t21', 'b2.t21')}
...
#functions to determine best distance (best wasserstein should be the smallest value, best diagonality should be the highest value)
...
for cat in categories:
	...
	for i, func in enumerate(funcs):
		...
		ws.cell(row=i*(n+3)+3, column=1).value = fn.upper() #func name
		ws.cell(row=i*(n+3)+3, column=1).font = bold_large

		ws.cell(row=i*(n+3)+4, column=1).value = 'Best' #best distance value for row
		ws.cell(row=i*(n+3)+4, column=1).border = bottom_line
		ws.cell(row=i*(n+3)+4, column=1).font = bold_small

		ws.cell(row=i*(n+3)+4, column=2).value = 'Error' #error between best and actual distance values
		ws.cell(row=i*(n+3)+4, column=2).border = bottom_line
		ws.cell(row=i*(n+3)+4, column=2).font = bold_small

		ws.cell(row=i*(n+3)+4, column=3).value = 'Error rel' #relative error
		...
		#calculate the distance data using function
		d = func(ir_dft, ir_dftb)
		
		#normalize data for colouring
