...
class ColourMap:
	...
	def get_hex_colours(self, array):
		r, g, b = self._colour_array(array)
		r = r.astype(int)
		g = g.astype(int)
		b = b.astype(int)
		carray = np.empty(array.shape, dtype='|S6')
		for i, _ in enumerate(r):
			for j, _ in enumerate(_):
				carray[i,j] = f'{r[i,j]:02x}{g[i,j]:02x}{b[i,j]:02x}'
	...
	def _colour_array(self, val, convert_to_pg=False):
		if self.posneg_mode:
