...
def sinkhorn(a, b, epsilon=0.4, cost_fn=None, error_fn=None, max_iter=100000, converge_thresh=10**-10, print_complexity=True):
    '''
    ...
    #calculate and print complexity:
    delta = np.max(np.count_nonzero(K,axis=0))
    nu = np.min(K)/np.max(K)
    print(f'Complexity: t = O(nln(Δ/ν)*(1/ε + 1/ε²) = {complexity}')
